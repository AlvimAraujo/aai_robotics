...
class RosiNodeClass():
    ...
    def callback_joints(self, data):
        ...
        if(self.touch == 1):
            ...
            self.desired_joint3 = 0.0
            self.desired_joint4 = 0.0
            if(abs(self.actual2 - 0) >= self.err and abs(self.actual3 - 0) >= self.err and abs(self.actual4 - 0) >= self.err):
                self.desired_joint1 = 0.0
                self.desired_joint5 = 0.0


        #if(abs(self.desired_joint1 - self.actual1) >= self.err):
        self.joint1 = self.desired_joint1
        #if(abs(self.desired_joint2 - self.actual2) >= self.err):
        self.joint2 = self.desired_joint2
        #if(abs(self.desired_joint3 - self.actual3) >= self.err):
        self.joint3 = self.desired_joint3
        #if(abs(self.desired_joint4 - self.actual4) >= self.err):
        self.joint4 = self.desired_joint4
        #if(abs(self.desired_joint5 - self.actual5) >= self.err):
        self.joint5 = self.desired_joint5
        #if(abs(self.desired_joint6 - self.actual6) >= self.err):
